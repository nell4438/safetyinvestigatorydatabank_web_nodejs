    ...
    <v-main>
        ...
        <v-container fluid>
            <v-card class="mt-3" v-if="isloading== false" :class="[isloading == false ? `imagehover` : '']">
                <v-row>
                    <v-col>
                        <v-card-subtitle class='ml-5  pb-7 mr-4 pt-5 ' style="font-size:20px">Property Damaged</v-card-subtitle>
                    </v-col>
                            ...
                            <v-form ref="form" v-model="valid" lazy-validation class="mt-4">
                                <v-row align="center" justify="center" dense>  
                                    <v-col v-if="editMode == true" cols="11">
                                        <v-text-field  
                                            v-model="toModify.PropertyDamagedID"                    
                                            label="ID"
                                ...
                                <v-row align="center" justify="center" dense> 
                                    <v-col cols="11">
                                        <v-text-field     
                                            v-model="toModify.PropertyDamaged"                  
                                            label="Property Damaged Name"  
                                            dense
                                            outlined         
                                        ></v-text-field>
                                    </v-col>
...
export default{
    data() {
        return {
            ...
            headers:[
                { text: "ID", value: "PropertyDamagedID" },
                { text: "Property Damaged Name", value: "PropertyDamaged" },
    ...
    mounted(){
    this.getPropertyDamaged();
    },

    computed:{
        filteredData(){
            if(!this.search.toLowerCase()){
                return this.PropertyDamagedItems
            }else{
                return this.PropertyDamagedItems.filter(rec=>{
                return rec.PropertyDamaged.includes(this.search)
                })
            }
        }, 
        GV_UserID: {
    ...
    methods: { 
        ...
        getPropertyDamaged(){
            this.isloading = true
            this.axios.get(`${this.api}/getpropertydamagedNULL`).then(res=>{
                this.PropertyDamagedItems=res.data
        ...
        saveRecords() {                       
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/updatepropertydamaged`,this.toModify).then(() => {
        ...
        addRecords(){
            this.toModify.PropertyDamagedID = this.PropertyDamagedItems.length + 1,
            ...
            this.swal.fire(this.saveOptions).then(result => {
            if(result.isConfirmed){
                this.axios.post(`${this.api}/addpropertydamaged`,this.toModify).then(() => {
                this.swal.fire('Hooray!','Changes has been saved', 'success')
                this.dialog=false
                this.editMode = false
                this.getPropertyDamaged() 
        ...
        deleteData(item) {
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/deletpropertydamaged`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getPropertyDamaged() 
        ...
        ActivateData(item){
            this.swal.fire(this.saveOptions).then(result => {
                if(result.isConfirmed){
                        this.axios.post(`${this.api}/activepropertydamaged`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getPropertyDamaged() 
