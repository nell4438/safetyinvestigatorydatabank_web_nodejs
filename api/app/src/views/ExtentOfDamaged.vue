    ...
    <v-main>
        ...
        <v-container fluid>
            <v-card class="mt-3" v-if="isloading== false" :class="[isloading == false ? `imagehover` : '']">
                <v-row>
                    <v-col>
                        <v-card-subtitle class='ml-5  pb-7 mr-4 pt-5 ' style="font-size:20px">Extent of Damaged</v-card-subtitle>
                    </v-col>
                        ...
                        <v-container>
                            <v-form ref="form" v-model="valid" lazy-validation class="mt-4">
                                ...
                                <v-row align="center" justify="center" dense> 
                                    <v-col cols="11">
                                        <v-text-field     
                                            v-model="toModify.ExtentofDamaged"                  
...
export default{
    data() {
        return {
            ...
            headers:[
                { text: "ID", value: "DamagedID" },
                { text: "Classification Name", value: "ExtentofDamaged" },
    ...
    mounted(){
    this.getExtentofDamaged();
    },

    computed:{
        filteredData(){
            if(!this.search.toLowerCase()){
                return this.ExtentofDamagedItems
            }else{
                return this.ExtentofDamagedItems.filter(rec=>{
                return rec.ExtendofDamaged.includes(this.search)
                })
            }
        }, 
        GV_UserID: {
    ...
    methods: { 
        ...
        getExtentofDamaged(){
            this.isloading = true
            this.axios.get(`${this.api}/getextentofdamagedNULL`).then(res=>{
                this.ExtentofDamagedItems=res.data
        ...
        saveRecords() {                       
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/updateextentofdamage`,this.toModify).then(() => {
        ...
        addRecords(){
            this.toModify.DamagedID = this.ExtentofDamagedItems.length + 1,
            ...
            this.swal.fire(this.saveOptions).then(result => {
            if(result.isConfirmed){
                this.axios.post(`${this.api}/addextentofdamaged`,this.toModify).then(() => {
                this.swal.fire('Hooray!','Changes has been saved', 'success')
                this.dialog=false
                this.editMode = false
                this.getExtentofDamaged() 
        ...
        deleteData(item) {
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/deleteextentofdamaged`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getExtentofDamaged() 
        ...
        ActivateData(item){
            this.swal.fire(this.saveOptions).then(result => {
                if(result.isConfirmed){
                        this.axios.post(`${this.api}/activeextentofdamaged`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getExtentofDamaged() 
