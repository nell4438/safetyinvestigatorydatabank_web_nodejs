  ...
  <v-main>
    ...
    <v-container fluid>
      <v-card
        ...
        class="mt-3"
        :class="[isloading == false ? `imagehover` : '']"
      >
        <v-row>
          <v-card-subtitle class="ml-7 pt-7 mr-4" style="font-size: 20px"
            >Investigatory Records</v-card-subtitle
          >
          <v-switch
            v-model="switchtable"
            rounded
            ...
            color="#03a89a"
          ></v-switch>
          <p v-if="switchtable == true" class="mt-7" style="color: gray">
            Active Records
          </p>
        ...
        >
          ...
          <template v-slot:[`item.TypeOfReports`]="{ item }">
            <v-chip :color="getColor(item.TypeOfReports)" dark small
              >{{ item.TypeOfReports }}
            </v-chip>
          </template>
        ...
        <v-data-table
          v-if="switchtable == false && isloading == false"
          dense
          :headers="tableHeader"
          :items="TableFilter2"
          :page.sync="page2"
          ...
          <v-tabs-items v-model="tab" class="mt-2 ml-2 mr-2 pr-2 pl-2 pt-2 pb-1">
            ...
            <v-tab-item value="2">
              <v-card class="pa-4 MyCard1">
                ...
                <v-row class="pt-0 mt-0">
                  ...
                  <v-col cols="2">
                    <v-text-field
                      v-model="editItems.LengthofExposure"
                ...
                <v-row class="pt-0 mt-0">
                  ...
                  <v-col cols="3">
                    <!-- :readonly=" editItems.EmployeeID == ''  ? false : true" -->
                    <!-- {{editItems.Department}} -->
                    <!-- {{DepartmentList}} -->
                    <!-- <v-autocomplete
                                    v-model="editItems.Department"
                                    label="Department Name"
                                    @change="GetSections(infoData.CompanyCode, editItems.Department.replaceAll(' ','%20'))"
            ...
            <v-tab-item value="3">
              <v-card class="pa-2 MyCard1">
                <p style="font-size: 11px; margin-right: 10px" align="right">
                  <a
                    href="http://hv12-hrd41:3001/ftp/FileServer/SafetyInvestigatoryDatabank/NoPicture.png"
                    >Download NO Picture Available here</a
                  >
                </p>
                  ...
                  <v-row>
                    <v-col style="margin-left: 100px" cols="5" align="center">
                      ...
                      <v-img
                        ...
                        v-if="this.editItems.PictureCause !== null"
                        :src="imageUrl"
                      >
                      </v-img>
                    ...
                    <v-col style="margin-left: 40px" cols="5" align="center">
                      ...
                      <v-img
                        style="border: 1px solid black; height: 300px; width: 430px"
                        v-if="this.editItems.PictureEffect !== null"
                        :src="imageUrl2"
                      >
                      </v-img>
...
export default {
  ...
  data() {
    return {
      ...
      altImage:
        "http://hv12-hrd41:3001/ftp/FileServer/SafetyInvestigatoryDatabank/NoImageFound.png",
      ...
      // companyHTI : 'FM-HTI 02-FSE-49; 2021/09/13',
      // companySCAD:'S-QR-1014-00; 2021/09/13',
      // companyWukong:'JF192-01; 2021/09/13',
      // companyHRD:'',
      // PV:'',
      // companyheader:'',
    };
  ...
  methods: {
    GetDepartments(companyname) {
      ...
      axios
        .get(
          `http://hrd-adminweb/api/company/department/${companyname}`,
          this.HeadersConfig
        )
        .then((res) => {
          this.DepartmentList = res.data.filter((rec) => {
            return rec.DeletedDate == null;
          });
          // console.log('DepartmentList',this.DepartmentList)
        });
    },
    GetSections(currentUserCompCode, currentUserDeptName) {
      axios
        .get(
          `http://hrd-adminweb/api/employees/${currentUserCompCode}/${currentUserDeptName}`,
          this.HeadersConfig
        )
        .then((res) => {
          this.SectionList = res.data
            .filter((rec) => {
              return rec.SeparationDate == null;
            })
            .map((r) => {
              return {
          ...
          // console.log('sect', this.SectionList)
        });
    },
    ...
    globalCompanyPath() {
      ...
      // console.log("PATH", this.CompanyPath);
    },
    GetSIRNo() {
      ...
      if (this.editItems.ReportID === 1) {
        this.tempSIRNo = this.tempSIRNo + "I";
      } else if (this.editItems.ReportID === 2) {
        this.tempSIRNo = this.tempSIRNo + "A";
      } else if (this.editItems.ReportID === 3) {
        this.tempSIRNo = this.tempSIRNo + "F";
      } else if (this.editItems.ReportID === 4) {
        this.tempSIRNo = this.tempSIRNo + "L";
      } else {
        this.tempSIRNo = "";
      }

      if (this.editItems.ReportID === 2 && this.editItems.DegreeofAccidentID === 1) {
        this.tempSIRNo = this.tempSIRNo + "J";
      } else if (
        this.editItems.ReportID === 2 &&
        this.editItems.DegreeofAccidentID === 2
      ) {
        this.tempSIRNo = this.tempSIRNo + "N";
      } else if (
        this.editItems.ReportID === 2 &&
        this.editItems.DegreeofAccidentID === 3
      ) {
        this.tempSIRNo = this.tempSIRNo + "";
      }
      if (this.editItems.ReportID === 1 && this.editItems.DegreeofAccidentID === 1) {
        this.tempSIRNo = this.tempSIRNo + "J";
      } else if (
        this.editItems.ReportID === 1 &&
        this.editItems.DegreeofAccidentID === 2
      ) {
        this.tempSIRNo = this.tempSIRNo + "N";
      } else if (
      ...
      if (this.editItems.ReportID === 3 && this.editItems.DegreeofAccidentID === 1) {
        this.tempSIRNo = this.tempSIRNo + "J";
      } else if (
        this.editItems.ReportID === 3 &&
        this.editItems.DegreeofAccidentID === 2
      ) {
        this.tempSIRNo = this.tempSIRNo + "N";
      } else if (
      ...
      if (this.editItems.ReportID === 4 && this.editItems.DegreeofAccidentID === 1) {
        this.tempSIRNo = this.tempSIRNo + "J";
      } else if (
        this.editItems.ReportID === 4 &&
        this.editItems.DegreeofAccidentID === 2
      ) {
        this.tempSIRNo = this.tempSIRNo + "N";
      } else if (
      ...
      if (
        this.editItems.ReportID === undefined ||
        this.editItems.ReportID === null ||
        this.editItems.ReportID === "" ||
        this.editItems.ReportID === 0 ||
        this.editItems.DegreeofAccidentID === undefined ||
        this.editItems.DegreeofAccidentID === null ||
        this.editItems.DegreeofAccidentID === "" ||
        this.editItems.DegreeofAccidentID === 0 ||
        this.editItems.DateHappened === undefined ||
        this.editItems.DateHappened === null ||
        this.editItems.DateHappened === "" ||
        this.editItems.DateHappened === 0 ||
        this.editItems.DateHappened ===
          moment(this.editItems.DateHappened).format("YYYYMMDD")
      ) {
        this.editItems.SirNo = undefined;
      } else {
        axios
          .get(
            `${this.api}/getseqno/${this.editItems.DateHappened}/${this.SIdatabank_userInfo.CompanyCode}`
          )
          .then((res) => {
            let Data = res.data[0];
            if (this.editItems.DateHappened !== this.tempDateHappened) {
              this.editItems.SeqID = Data.SeqID;
              this.editItems.SirNo =
                this.tempSIRNo +
                moment(this.editItems.DateHappened).format("YYYYMMDD") +
                "-" +
                Data.SeqID;
    ...
    EmployeeInfo() {
      let tempEmployeeCode = this.editItems.EmployeeID;
      if (tempEmployeeCode != "") {
        axios
          .get(`${this.EmployeeInfoURL}/${this.editItems.EmployeeID}`, this.HeadersConfig)
          .then((res) => {
            this.infoData = res.data[0];

            if (tempEmployeeCode != "" && this.infoData != undefined) {
              this.GetDepartments(this.infoData.CompanyCode);
              // console.log('info',this.infoData.CompanyCode)

              if (this.infoData.DepartmentName != undefined) {
                let modifiedDeptName = this.infoData.DepartmentName.replaceAll(
                  " ",
                  "%20"
                );
                if (modifiedDeptName != undefined) {
                  this.GetSections(this.infoData.CompanyCode, modifiedDeptName);
                  // console.log('DATA', this.infoData.CompanyCode,modifiedDeptName)
                }
              }
            }
            ...
            if (
              ...
              this.editItems.EmployeeAge = 0;
              this.editItems.Department = "";
              this.editItems.Section = "";
              this.editItems.LengthofService = "";
              this.editItems.CivilStausID = 0;
              this.editItems.ShiftID = 0;
            }
            // else {

            //     this.editItems.Name = this.infoData.EmployeeName
            //     this.editItems.Status= this.infoData.EmployeeStatus
            //     this.editItems.Position = this.infoData.PositionName
            //     this.editItems.Gender= this.infoData.Gender
            //     this.editItems.EmployeeAge = moment().diff(this.infoData.DateBirth,'years')
            //     this.editItems.Department = this.infoData.DepartmentName
            //     this.editItems.Section = this.infoData.SectionName
            //     this.editItems.LengthofService = moment().diff(this.infoData.DateHired,'years')
            //     this.editItems.DepartmentCode = this.infoData.DepartmentCode
            //     // this.editItems.LengthofExposure =
            // }
          });
      } else {
        this.editItems.Name = "";
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          axios.post(url, item).then(() => {
            Swal.fire("Unlocked!", `${item.SirNo} has been unlocked`, "success");
          });
          setTimeout(() => {
            this.onLoadData();
          }, 500);
    ...
    lockRecord(item) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          axios.post(url, item).then(() => {
            Swal.fire("Locked!", `${item.SirNo} has been locked`, "success");
          });
          setTimeout(() => {
            this.onLoadData();
          }, 500);
    ...
    async yearfilter() {
      ...
      await axios
        .get(
          `${this.api}/getTableData/${this.SIdatabank_userInfo.CompanyCode}/${this.filterYear}`
        )
        .then((res) => {
    ...
    async onLoadData() {
      ...
      await axios
        .get(
          `${this.EmployeeInfoURL}/${this.SIdatabank_userInfo.EmployeeID}`,
          this.HeadersConfig
        )
        .then((res) => {
      ...
      await axios
        ...
        .then((res) => {
          this.tableItems = res.data;
          // .filter(async r=>{
          //${this.ftpPath}/${this.CompanyPath}/${r.SirNo}/${r.PictureCause}

          //     return r
          // })
          // console.log('data',this.tableItems)

          this.tableItems.forEach(async (Item) => {
            ...
            if (
              ...
              axios
                .get(`${this.EmployeeInfoURL}/${EmployeeID}`, this.HeadersConfig)
                .then((res) => {
            ...
            if (
              ...
              DepartmentHeadCode != undefined &&
              DepartmentHeadCode != "" &&
              DepartmentHeadCode != " "
            ) {
              axios
                .get(`${this.EmployeeInfoURL}/${DepartmentHeadCode}`, this.HeadersConfig)
                .then((res) => {
                  let EmployeeData = res.data[0];
            ...
            if (
              ...
              ImmediateSuperiorCode != undefined &&
              ImmediateSuperiorCode != "" &&
              ImmediateSuperiorCode != " "
            ) {
              axios
                .get(
                  `${this.EmployeeInfoURL}/${ImmediateSuperiorCode}`,
                  this.HeadersConfig
                )
                .then((res) => {
                  let EmployeeData = res.data[0];
            ...
            if (
              DepartmentAdviserName != null &&
              DepartmentAdviserName != undefined &&
              DepartmentAdviserName != "" &&
              DepartmentAdviserName != " "
            ) {
              axios
                .get(
                  `${this.EmployeeInfoURL}/${DepartmentAdviserName}`,
                  this.HeadersConfig
                )
                .then((res) => {
                  let EmployeeData = res.data[0];
            ...
            if (
              ...
              InvestigatorCode != undefined &&
              InvestigatorCode != "" &&
              InvestigatorCode != " "
            ) {
              axios
                .get(`${this.EmployeeInfoURL}/${InvestigatorCode}`, this.HeadersConfig)
                .then((res) => {
                  let EmployeeData = res.data[0];
            ...
            if (
              SafetyTeamLeaderCode != null &&
              SafetyTeamLeaderCode != undefined &&
              SafetyTeamLeaderCode != "" &&
              SafetyTeamLeaderCode != " "
            ) {
              axios
                .get(
                  `${this.EmployeeInfoURL}/${SafetyTeamLeaderCode}`,
                  this.HeadersConfig
                )
                .then((res) => {
                  let EmployeeData = res.data[0];
            ...
            if (
              FSQDepartmentHeadCode != null &&
              FSQDepartmentHeadCode != undefined &&
              FSQDepartmentHeadCode != "" &&
              FSQDepartmentHeadCode != " "
            ) {
              axios
                .get(
                  `${this.EmployeeInfoURL}/${FSQDepartmentHeadCode}`,
                  this.HeadersConfig
                )
                .then((res) => {
                  let EmployeeData = res.data[0];
            ...
            if (
              FSQDepartmentAdviserName != null &&
              FSQDepartmentAdviserName != undefined &&
              FSQDepartmentAdviserName != "" &&
              FSQDepartmentAdviserName != " "
            ) {
              axios
                .get(
                  `${this.EmployeeInfoURL}/${FSQDepartmentAdviserName}`,
                  this.HeadersConfig
                )
                .then((res) => {
                  let EmployeeData = res.data[0];
            ...
            //ADDED
            Item.PictureCause = Item.PictureCause.replace(
              "//hv12-hrd41/D/Deployment/SafetyInvestigatoryDatabank",
              "http://hv12-hrd41:3001/ftp/FileServer/SafetyInvestigatoryDatabank"
            );
            Item.PictureEffect = Item.PictureEffect.replace(
              "//hv12-hrd41/D/Deployment/SafetyInvestigatoryDatabank",
              "http://hv12-hrd41:3001/ftp/FileServer/SafetyInvestigatoryDatabank"
            );

            //  Item.hold1 =await this.getBase64ImageFromURL(Item.PictureCause);
            //  Item.hold2 =await this.getBase64ImageFromURL(Item.PictureEffect);

            await this.getBase64ImageFromURL(`${Item.PictureCause}`).then((response) => {
              Item.hold1 = response;
            });

            await this.getBase64ImageFromURL(`${Item.PictureEffect}`).then((response) => {
              Item.hold2 = response;
            });
          });
          this.isloading = false;
        });
      // http://hv12-hrd41:3001/ftp/FileServer/SafetyInvestigatoryDatabank/SCD/SAJ20230530-1/
      // //hv12-hrd41/D/Deployment/SafetyInvestigatoryDatabank/SCD/SAJ20230530-1/SAJ20230530-1-C.png
    ...
    async onLoadData2() {
      this.isloading = true;
      await axios
        .get(`${this.api}/getTableData2/${this.SIdatabank_userInfo.CompanyCode}`)
        .then((res) => {
    ...
    deletePermanentRecord2(item) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          axios.post(url, item).then(() => {
            Swal.fire("Deleted!", `${item.SirNo} has been deleted`, "success");
            this.onLoadData();
            this.onLoadData2();
          });
        }
    ...
    async UpdateRecord() {
      ...
      this.editItems.FurtherInvestigation = this.editItems.FurtherInvestigation.replace(
        /'/g,
        "''"
      );
      this.editItems.Conclusion = this.editItems.Conclusion.replace(/'/g, "''");
      this.editItems.CorrectiveAction = this.editItems.CorrectiveAction.replace(
        /'/g,
        "''"
      );
      this.editItems.RootCauseAnalysis = this.editItems.RootCauseAnalysis.replace(
        /'/g,
        "''"
      );
      ...
      this.editItems.ImmediateActionTaken = this.editItems.ImmediateActionTaken.replace(
        /'/g,
        "''"
      );
      ...
      if (this.oldSIRNo !== this.editItems.SirNo) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          if (this.defaultImage == true) {
            this.imageUrl = this.tempimage;
            this.imageUrl2 = this.tempimage;
          }
          // var sizeof = require("object-sizeof");

          // console.log('sizeof',sizeof(JSON.parse(JSON.stringify(this.editItems))));

          const url = `${this.api}/updatesafetyrecords`;

          // console.log('edit items',JSON.parse(JSON.stringify(this.editItems)) )
    ...
    async EditReportButton(item) {
      this.isloading = true;
      if (this.GroupOfCompaniesnoFilter.length == 0) {
        await this.GetGroupOfCompaniesnoFilter();
      }
    ...
    async DepartmentSearch() {
      await axios.get(`${this.DepartmentURL}/${this.CompanyPath}`).then((res) => {
    ...
    GetGroupOfCompanies() {
      axios
        .get(
          `${this.leadersURL}/${this.editItems.selectcompany}/${this.editItems.departmentcode}`
        )
        .then((res) => {
    ...
    async GetGroupOfCompaniesnoFilter() {
      await axios
        .get(`${this.leadersURL}/${this.SIdatabank_userInfo.CompanyCode}`)
        .then((res) => {
    ...
    PromiseGetJapaneseAdvisers() {
      return new Promise((resolve) => {
        ...
        axios
          .get(`${this.api}/getJapAdvisers/${this.strConvertedJapaneseCompanyCode}`)
          .then((res) => {
            let toSend = res.data.map((rec) => {
              return {
                text: rec.EmployeeName,
    ...
    PromiseWorkshift() {
      return new Promise((resolve) => {
        axios
          .get(`${this.api}/getworkshift/${this.SIdatabank_userInfo.CompanyCode}`)
          .then((res) => {
            let toSend = res.data.map((rec) => {
              return {
                text: rec.Time,
    ...
    PromiseGetYear() {
      return new Promise((resolve) => {
        axios
          .get(`${this.api}/getyear/${this.SIdatabank_userInfo.CompanyCode}`)
          .then((res) => {
            let toSend = res.data.map((rec) => {
              return {
                text: rec.Year,
    ...
    async SirReportButton(item) {
      if (this.SIdatabank_userInfo.CompanyCode === "00") {
        ...
        this.companyheader = "S-QR-1014-00 (11) ;September 16,2021";
        // this.companyheader = 'S-QR-1014-00; 2021/09/16'
      } else if (this.SIdatabank_userInfo.CompanyCode === "20") {
        // this.companyheader = 'FM-HTI 02-FSE-49 (11) ;September 16,2021' // old document code
        this.companyheader = "FM-HTI 02-FSE-49 (12) ;September 24,2022"; // new document code 2023-04-13
        this.Retention = "Retention Period: 1 Year";
      } else if (this.SIdatabank_userInfo.CompanyCode === "30") {
        this.companyheader = "";
      } else if (this.SIdatabank_userInfo.CompanyCode === "40") {
        this.companyheader = "JF192-01 (11) ;September 16,2021";
        // this.companyheader = 'JF192-01; 2021/09/16'
      }

      let self = this;

      fetch(`${this.ftpPath}/${this.CompanyPath}/${item.SirNo}/${this.PictureEffect}`, {
        method: "HEAD",
      })
        .then((res) => {
          if (!res.ok) {
            ...
            Toast.fire({
              icon: "warning",
              title: `Please Reupload Picture Effect Image`,
            });
          }
        })
        .catch((err) => console.log("Error:", err));
      fetch(`${this.ftpPath}/${this.CompanyPath}/${item.SirNo}/${this.PictureCause}`, {
        method: "HEAD",
      })
        .then((res) => {
          if (!res.ok) {
            ...
            Toast.fire({
              icon: "warning",
              title: `Please Reupload Picture Cause Image`,
            });
          }
        })
        .catch((err) => console.log("Error:", err));
      var SIRReport = {
        pageMargins: [20, 35, 20, 25],
        content: [
          //  { text: this.companyheader,
...

          {
            columns: [
              ...
              {
                text: `: ${
                  item.DateHappened != null || item.DateHappened != undefined
                    ? item.DateHappened
                    : ""
                }     ${
                  item.TimeHappened != null || item.TimeHappened != undefined
                    ? item.TimeHappened
                    : ""
                }`,
                margin: [-40, 5, 0, 0],
              ...
              {
                text: `: ${
                  item.PlaceofIncident.replace(/''/g, "'") != null ||
                  item.PlaceofIncident.replace(/''/g, "'") != undefined
...

          {
            columns: [
              ...
              {
                text: `: ${
                  item.AccidentDegree != null || item.AccidentDegree != undefined
                    ? item.AccidentDegree
                    : ""
                }`,
                italics: true,
              ...
              {
                text: `: ${
                  item.InjuryLevel != null || item.InjuryLevel != undefined
                    ? item.InjuryLevel
                    : ""
                }       Day Lost: ${
                  item.DaysLost != null || item.DaysLost != undefined ? item.DaysLost : ""
                } `,
                italics: true,
                margin: [-67, 5, 0, 0],
                fontSize: 8,
...

          {
            columns: [
              ...
              {
                text: `: ${
                  item.LengthofService != null || item.LengthofService != undefined
            ...
            columns: [
...

              {
                text: `: ${
                  ...
                  item.EmployeeAge != null || item.EmployeeAge != undefined
                    ? item.EmployeeAge
                    : ""
                }`,
                margin: [-40, 5, 0, 0],
              ...
              {
                text: `: ${
                  item.LengthofExposure != null || item.LengthofExposure != undefined
                    ? item.LengthofExposure
                    : ""
                }`,
                margin: [-68, 5, 0, 0],
                italics: true,
...

          {
            columns: [
              ...
              {
                text: `: ${
                  item.NatureOfInjury != null || item.NatureOfInjury != undefined
                    ? item.NatureOfInjury
                    : ""
                }`,
                margin: [-40, 5, 0, 0],
              ...
              {
                text: `: ${
                  item.ExtentOfInjuries != null || item.ExtentOfInjuries != undefined
                    ? item.ExtentOfInjuries
                    : ""
                }`,
                margin: [-68, 5, 0, 0],
                italics: true,
...

          {
            columns: [
              ...
              {
                text: `: ${
                  item.PartsInjuredName != null || item.PartsIjurnedName != undefined
                    ? item.PartsInjuredName
                    : ""
                } `,
                margin: [-40, 5, 0, 0],
                italics: true,
              ...
              {
                text: `: ${
                  item.PrimaryTME != null || item.PrimaryTME != undefined
                    ? item.PrimaryTME
                    : ""
                }`,
                margin: [-68, 5, 0, 0],
...

          {
            columns: [
              ...
              {
                text: `: ${
                  item.SpecificParts != null || item.SpecificParts != undefined
                    ? item.SpecificParts
                    : ""
                }`,
                margin: [-40, 5, 0, 0],
              ...
              {
                text: `: ${
                  item.PartsTME != null || item.PartsTME != undefined ? item.PartsTME : ""
                }`,
                margin: [-68, 5, 0, 0],
                italics: true,
                fontSize: 8,
...

          {
            columns: [
              ...
              {
                text: `: ${
                  item.UnsafeAction != null || item.UnsafeAction != undefined
                    ? item.UnsafeAction
                    : ""
                }`,
                margin: [-40, 5, 25, 0],
              ...
              {
                text: `: ${
                  item.UnsafeCondition != null || item.UnsafeCondition != undefined
                    ? item.UnsafeCondition
                    : ""
                } \n \n \n`,
                margin: [-68, 5, 0, 0],
                italics: true,
          ...
          {
            text: `${
              item.Details != null || item.Details.replace(/''/g, "'") != undefined
            ...
            columns: [
              {
                image: await this.getBase64ImageFromURL(
                  `${this.ftpPath}/${this.CompanyPath}/${item.SirNo}/${this.PictureCause}`
                ),
                margin: [50, 0, 0, 0],
                width: 200,
                height: 155,
              ...
              {
                image: await this.getBase64ImageFromURL(
                  `${this.ftpPath}/${this.CompanyPath}/${item.SirNo}/${this.PictureEffect}`
                ),
                margin: [104, 0, 0, 0],
                width: 200,
                height: 155,
          ...
          {
            text: `${
              item.ImmediateActionTaken.replace(/''/g, "'") != null ||
              item.ImmediateActionTaken.replace(/''/g, "'") != undefined
                ? item.ImmediateActionTaken
                : ""
            } \n\n`,
            margin: [11, 9, 0, 5],
            fontSize: 9,
          ...
          {
            text: `${
              item.FurtherInvestigation.replace(/''/g, "'") != null ||
              item.FurtherInvestigation.replace(/''/g, "'") != undefined
                ? item.FurtherInvestigation
                : ""
            } \n\n`,
            alignment: "justify",
            margin: [11, 9, 0, 5],
          ...
          {
            text: `${
              item.RootCauseAnalysis.replace(/''/g, "'") != null ||
              item.RootCauseAnalysis.replace(/''/g, "'") != undefined
          ...
          {
            text: `${
              item.CorrectiveAction.replace(/''/g, "'") != null ||
              item.CorrectiveAction.replace(/''/g, "'") != undefined
          ...
          {
            text: `${
              item.Conclusion.replace(/''/g, "'") != null ||
              item.Conclusion.replace(/''/g, "'") != undefined
                ? item.Conclusion
                : ""
            }\n\n\n\n`,
            margin: [11, 9, 0, 15],
          ...
          },
          {
            columns: [
              {
                text: `Investigated by:     ${
                  item.InvestigatorCode != null || item.InvestigatorCode != undefined
                    ? item.InvestigatorCode
                    : ""
                }`,
                margin: [15, 9, 0, 0],
                fontSize: 10,
              ...
              {
                text: `Checked by:     ${
                  item.SafetyTeamLeaderCode != null ||
                  item.SafetyTeamLeaderCode != undefined
                    ? item.SafetyTeamLeaderCode
                    : ""
                }`,
                margin: [10, 9, 0, 0],
                fontSize: 10,
          ...
          },
          {
            columns: [
              ...
              {
                text: `Occupational Safety TH(T)/SH(T)`,
                margin: [50, 0, 0, 30],
                fontSize: 10,
                bold: true,
                decoration: "underline",
          ...
          },
          {
            columns: [
              {
                text: `Approved by:     ${
                  item.DepartmentHead != null || item.DepartmentHead != undefined
                    ? item.DepartmentHead
                    : ""
                }`,
                margin: [15, 9, 0, 0],
              ...
              {
                text: `Approved by:     ${
                  item.FSQDepartmentHeadCode != null ||
                  item.FSQDepartmentHeadCode != undefined
                    ? item.FSQDepartmentHeadCode
                    : ""
                }`,
                margin: [10, 9, 0, 0],
                fontSize: 10,
          ...
          },
          {
            columns: [
              {
                text: `${
                  item.DepartmentAdviserName != null ||
                  item.DepartmentAdviserName != undefined
                    ? item.DepartmentAdviserName
                    : ""
                }`,
                margin: [40, 9, 0, 0],
                fontSize: 10,
              ...
              {
                text: `${
                  item.FSQDepartmentAdviserName != null ||
                  item.FSQDepartmentAdviserName != undefined
                    ? item.FSQDepartmentAdviserName
                    : ""
                }`,
                margin: [-50, 9, 0, 0],
                fontSize: 10,
                  ...
                  [
                    {
                      ...
                      fontSize:
                        item.CompanyName == "House Technology Industries Pte. Ltd."
                          ? 9
                          : 10,
                      //  margin:[10,0,0,0]
                      //  margin:[135,0,0,0]
                      margin:
                        item.CompanyName == "House Technology Industries Pte. Ltd."
                          ? [135, -7, 0, 0]
                          : [170, 0, 0, 0] ||
                            item.CompanyName == "SCAD SERVICES  Singapore PTE. LTD."
                          ? [135, 0, 0, 0]
                          : [170, 0, 0, 0],
                    },
    ...
    async FlashReportButton(item) {
      // console.log('Tama ba to?',FlashReport)
      ...
      var FlashReport = {
        ...
        content: [
...

          {
            columns: [
              ...
              {
                text: `${
                  item.TypeOfReports != null || item.TypeOfReports != undefined
                    ? item.TypeOfReports
                    : ""
                }`,
                margin: [-35, 5, 0, 0],
...

          {
            columns: [
              ...
              {
                text: `${
                  item.ImmediateSuperior != null || item.ImmediateSuperior != undefined
                    ? item.ImmediateSuperior
                    : ""
                }`,
                margin: [-60, 5, 0, 0],
                italics: true,
          ...
          {
            text: `Safety Staff/Safety Officer`,
            decoration: "overline",
            margin: [11, 0, 0, 0],
            fontSize: 10,
          },
        ...
        footer: function (currentPage, pageCount) {
          // console.log('QWRTY',item)
          if (
            item.TypeOfReports == "Incident" ||
            item.TypeOfReports == "Accident" ||
            item.TypeOfReports == "Forklift Incident"
          ) {
            return {
              table: {
                body: [
                  [
                    {
                      text:
                        "Level D = 0 lost day : Level C = 1-4 lost days : Level B = 5-14 lost days : Level A = 15-30 lost days : Level S =  more than 30 lost days\n",
                      margin: [20, 0, 0, 50],
                      fontSize: 8,
                      italics: "true",
                    },
          ...
          if (item.TypeOfReports == "Fire Incident") {
            return {
              table: {
                body: [
                  [
                    {
                      text:
                        "Level D = No FFE use : Level B = Fire Extinguisher : Level A = Fire Hydrant/Sprinkler System : Level S =  Fire Truck\n",
                      margin: [20, 0, 0, 50],
                      fontSize: 8,
                      italics: "true",
                    },
      ...
      await pdfMake.createPdf(FlashReport).open();
      // pdfMake.createPdf(FlashReport).download()
      // console.log('FlashReport', FlashReport)
    },
  },
