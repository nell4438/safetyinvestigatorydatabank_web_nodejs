    ...
    <v-main>
        ...
        <v-container fluid>
            <v-card class="mt-3" v-if="isloading== false" :class="[isloading == false ? `imagehover` : '']">
                <v-row>
                    <v-col>
                        <v-card-subtitle class='ml-5  pb-7 mr-4 pt-5 ' style="font-size:20px">Parts Injured</v-card-subtitle>
                    </v-col>
                            ...
                            <v-form ref="form" v-model="valid" lazy-validation class="mt-4">
                                <v-row align="center" justify="center" dense>  
                                    <v-col v-if="editMode == true" cols="11">
                                        <v-text-field  
                                            v-model="toModify.PartsInjuredID"                    
                                            label="ID"
                                ...
                                <v-row align="center" justify="center" dense> 
                                    <v-col cols="11">
                                        <v-text-field     
                                            v-model="toModify.PartsInjuredName"                  
...
export default{
    data() {
        return {
            ...
            headers:[
                { text: "ID", value: "PartsInjuredID" },
                { text: "Classification Name", value: "PartsInjuredName" },
    ...
    mounted(){
    this.getPartsInjured();
    },

    computed:{
        filteredData(){
            if(!this.search.toLowerCase()){
                return this.PartsInjuredItems
            }else{
                return this.PartsInjuredItems.filter(rec=>{
                return rec.PartsInjuredName.includes(this.search)
                })
            }
        }, 
        GV_UserID: {
    ...
    methods: { 
        ...
        getPartsInjured(){
            this.isloading = true
            this.axios.get(`${this.api}/getpartsinjuredNULL`).then(res=>{
                this.PartsInjuredItems=res.data
        ...
        saveRecords() {                       
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/updatepartsinjured`,this.toModify).then(() => {
        ...
        addRecords(){
            this.toModify.PartsInjuredID = this.PartsInjuredItems.length + 1,
            ...
            this.swal.fire(this.saveOptions).then(result => {
            if(result.isConfirmed){
                this.axios.post(`${this.api}/addinpartsinjured`,this.toModify).then(() => {
                this.swal.fire('Hooray!','Changes has been saved', 'success')
                this.dialog=false
                this.editMode = false
                this.getPartsInjured() 
        ...
        deleteData(item) {
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/deletepartsinjured`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getPartsInjured() 
        ...
        ActivateData(item){
            this.swal.fire(this.saveOptions).then(result => {
                if(result.isConfirmed){
                        this.axios.post(`${this.api}/activepartsinjured`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getPartsInjured() 
