    ...
    <v-main>
        ...
        <v-container fluid>
            <v-card class="mt-3" v-if="isloading== false" :class="[isloading == false ? `imagehover` : '']">
                <v-row>
                    <v-col>
                        <v-card-subtitle class='ml-5  pb-7 mr-4 pt-5 ' style="font-size:20px">Injury Level</v-card-subtitle>
                    </v-col>
                        ...
                        <v-container>
                            <v-form ref="form" v-model="valid" lazy-validation class="mt-4">
                                ...
                                <v-row align="center" justify="center" dense> 
                                    <v-col cols="11">
                                        <v-text-field     
                                            v-model="toModify.InjuryLevel"                  
                                            label="Injury Level Name"  
                                            dense
                                            outlined         
                                        ></v-text-field>
                                    </v-col>
...
export default{
    data() {
        return {
            ...
            headers:[
                { text: "ID", value: "InjuryID" },
                { text: "Injury Level", value: "InjuryLevel" },
    ...
    computed:{
        filteredData(){
            if(!this.search.toLowerCase()){
                return this.InjuryLevelItems
            }else{
                return this.InjuryLevelItems.filter(rec=>{
                return rec.InjuryLevel.includes(this.search)
                })
            }
        }, 
        GV_UserID: {
    ...
    methods: { 
        ...
        geInjuryLevels(){
            this.isloading = true
            this.axios.get(`${this.api}/getinjurylevelNULL`).then(res=>{
                this.InjuryLevelItems=res.data
        ...
        saveRecords() {                       
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/updateinjurylevel`,this.toModify).then(() => {
        ...
        addRecords(){
            this.toModify.InjuryID = this.InjuryLevelItems.length + 1,
            ...
            this.swal.fire(this.saveOptions).then(result => {
            if(result.isConfirmed){
                this.axios.post(`${this.api}/addinjurylevel`,this.toModify).then(() => {
                this.swal.fire('Hooray!','Changes has been saved', 'success')
                this.dialog=false
                this.editMode = false
                this.geInjuryLevels() 
        ...
        deleteData(item) {
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/deleteinjurylevel`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.geInjuryLevels() 
        ...
        ActivateData(item){
            this.swal.fire(this.saveOptions).then(result => {
                if(result.isConfirmed){
                        this.axios.post(`${this.api}/activeinjurylevel`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.geInjuryLevels() 
