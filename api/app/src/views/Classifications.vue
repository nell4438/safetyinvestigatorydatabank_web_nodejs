    ...
    <v-main>
        ...
        <v-container fluid>
            <v-card class="mt-3" v-if="isloading== false" :class="[isloading == false ? `imagehover` : '']">
                <v-row>
                    <v-col>
                        <v-card-subtitle class='ml-5  pb-7 mr-4 pt-5 ' style="font-size:20px">Classifications</v-card-subtitle>
                    </v-col>
                            ...
                            <v-form ref="form" v-model="valid" lazy-validation class="mt-4">
                                <v-row align="center" justify="center" dense>  
                                    <v-col v-if="editMode == true" cols="11">
                                        <v-text-field  
                                            v-model="toModify.ClassificationID"                    
                                            label="ID"
                                ...
                                <v-row align="center" justify="center" dense> 
                                    <v-col cols="11">
                                        <v-text-field     
                                            v-model="toModify.ClassificationName"                  
...
export default{
    data() {
        return {
            ...
            headers:[
                { text: "ID", value: "ClassificationID" },
    ...
    computed:{
        filteredData(){
            if(!this.search.toLowerCase()){
                ...
                return this.Classifications.filter(rec=>{
                return rec.ClassificationName.includes(this.search)
                })
            }
        }, 
        GV_UserID: {
    ...
    methods: { 
        ...
        getClassifications(){
            this.isloading = true
            this.axios.get(`${this.api}/getclassificationsNULL`).then(res=>{
        ...
        saveRecords() {                       
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/updateclassification`,this.toModify).then(() => {
        ...
        addRecords(){
            this.toModify.ClassificationID = this.Classifications.length + 1,
            ...
            this.swal.fire(this.saveOptions).then(result => {
            if(result.isConfirmed){
                this.axios.post(`${this.api}/addclassification`,this.toModify).then(() => {
                this.swal.fire('Hooray!','Changes has been saved', 'success')
                this.dialog=false
                this.editMode = false
                this.getClassifications() 
        ...
        deleteData(item) {
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/deleteclassification`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getClassifications() 
        ...
        ActivateData(item){
            this.swal.fire(this.saveOptions).then(result => {
                if(result.isConfirmed){
                        this.axios.post(`${this.api}/activateclassification`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getClassifications() 
