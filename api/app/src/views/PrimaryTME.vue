    ...
    <v-main>
        ...
        <v-container fluid>
            <v-card class="mt-3" v-if="isloading== false" :class="[isloading == false ? `imagehover` : '']">
                <v-row>
                    <v-col>
                        <v-card-subtitle class='ml-5  pb-7 mr-4 pt-5 ' style="font-size:20px">Primary TME</v-card-subtitle>
                    </v-col>
...
export default{
    data() {
        return {
            ...
            headers:[
                { text: "ID", value: "PrimaryTMEID" },
                { text: "Classification Name", value: "PrimaryTME" },
    ...
    computed:{
        filteredData(){
            if(!this.search.toLowerCase()){
                return this.PrimaryTMEItems
            }else{
                return this.PrimaryTMEItems.filter(rec=>{
                return rec.PrimaryTME.includes(this.search)
                })
            }
        }, 
        GV_UserID: {
    ...
    methods: { 
        ...
        getPrimaryTME(){
            this.isloading = true
            this.axios.get(`${this.api}/getprimarytmeNULL`).then(res=>{
                this.PrimaryTMEItems=res.data
        ...
        saveRecords() {                       
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/updateprimarytme`,this.toModify).then(() => {
        ...
        addRecords(){
            this.toModify.PrimaryTMEID = this.PrimaryTMEItems.length + 1,
            console.log(this.toModify.PrimaryTMEID)
            ...
            this.swal.fire(this.saveOptions).then(result => {
            if(result.isConfirmed){
                this.axios.post(`${this.api}/addprimarytme`,this.toModify).then(() => {
                this.swal.fire('Hooray!','Changes has been saved', 'success')
                this.dialog=false
                this.editMode = false
                this.getPrimaryTME() 
        ...
        deleteData(item) {
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/deleteprimarytme`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getPrimaryTME() 
        ...
        ActivateData(item){
            this.swal.fire(this.saveOptions).then(result => {
                if(result.isConfirmed){
                        this.axios.post(`${this.api}/activeprimarytme`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getPrimaryTME() 
