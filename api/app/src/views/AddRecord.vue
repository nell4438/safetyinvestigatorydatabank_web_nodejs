    ...
    <v-container>
      ...
      <v-card style="margin-left: auto; margin-right: auto">
        <v-sheet class="v-sheet--offset mb-6" color="#03a89a" height="60">
          <div>
            <v-card-subtitle
              style="font-size: 18px; margin-left: 8px; padding-top: 20px; color: #ffffff"
              >General Information</v-card-subtitle
            >
          </div>
        </v-sheet>
    ...
    <v-container>
      <v-card style="margin-left: auto; margin-right: auto; margin-top: 13px">
        <v-sheet class="v-sheet--offset mb-6" color="#03a89a" height="60">
          <v-row>
            <v-col>
              <v-card-subtitle
                class="pt-2"
                style="font-size: 18px; margin-left: 8px; color: #ffffff"
              >
                Person Involved
              </v-card-subtitle>
            </v-col>
        ...
        <v-form class="pl-8 pr-8" style="margin-bottom: 20px">
          ...
          <v-row class="pt-0 mt-0">
            ...
            <v-col cols="2">
              <v-text-field v-model="gender" label="Gender" outlined dense></v-text-field>
            </v-col>
            <v-col cols="3">
              ...
              <!-- <v-autocomplete
                                ...
                                @change="GetSections(infoData.CompanyCode, departmentname.replaceAll(' ','%20'))"
            ...
            <v-col>
              <v-card-subtitle
                ...
                style="
                  ...
                  color: #ffffff;
                "
                >Illustrations / Pictures</v-card-subtitle
              >
            </v-col>
            <v-col class="pt-4">
              <v-tooltip top>
      ...
      <v-card style="margin-left: auto; margin-right: auto; margin-top: 25px">
        ...
        <v-form class="pl-8 pr-8" style="margin-bottom: 20px">
          <v-row class="pt-0 mt-0">
            <v-col cols="6">
              <v-text-field v-model="witness1" label="Witness 1" outlined dense clearable>
              </v-text-field>
            </v-col>
            <v-col cols="6">
              <v-text-field
          ...
          <v-row class="pt-0 mt-0">
            <v-col cols="3">
              <v-autocomplete
                ...
                "
                @change="
                  crrdetails = crrviolationitems
                    .filter((rec) => {
                      return rec.CRRID == crrviolation;
                    })
                    .map((rec1) => {
                      return rec1.CRRDetails;
                    })[0]
                "
                label="CRR Violation"
                outlined
...
export default {
  ...
  data() {
    return {
      ...
      nameRules2: [(v) => !!v || "This field is required"],
      nameRules3: [(v) => !!v || "This field is required"],
  ...
  methods: {
    GetDepartments(companyShortName) {
      axios
        .get(
          `http://10.168.69.41/api/company/department/${companyShortName}`,
          this.HeadersConfig
        )
        .then((res) => {
    ...
    GetSections(currentUserCompCode, currentUserDeptName) {
      ...
      //    console.log('infoDATAAAAAAAAA', currentUserCompCode,currentUserDeptName)
      axios
        .get(
    ...
    PromiseGetSIRNo() {
      return new Promise((resolve) => {
        ...
        if (this.strTypeOfReport === 1) {
          this.tempSIRNo = this.tempSIRNo + "I";
        } else if (this.strTypeOfReport === 2) {
          this.tempSIRNo = this.tempSIRNo + "A";
        } else if (this.strTypeOfReport === 3) {
          this.tempSIRNo = this.tempSIRNo + "F";
        } else if (this.strTypeOfReport === 4) {
          this.tempSIRNo = this.tempSIRNo + "L";
        } else {
          this.tempSIRNo = "";
        }

        if (this.strTypeOfReport === 2 && this.strAccidentDegree === 1) {
          this.tempSIRNo = this.tempSIRNo + "J";
        } else if (this.strTypeOfReport === 2 && this.strAccidentDegree === 2) {
          this.tempSIRNo = this.tempSIRNo + "N";
        } else if (this.strTypeOfReport === 2 && this.strAccidentDegree === 3) {
          this.tempSIRNo = this.tempSIRNo + "";
        }
        if (this.strTypeOfReport === 1 && this.strAccidentDegree === 1) {
          this.tempSIRNo = this.tempSIRNo + "J";
        } else if (this.strTypeOfReport === 1 && this.strAccidentDegree === 2) {
          this.tempSIRNo = this.tempSIRNo + "N";
        } else if (this.strTypeOfReport === 1 && this.strAccidentDegree === 3) {
          this.tempSIRNo = this.tempSIRNo + "";
        }
        if (this.strTypeOfReport === 3 && this.strAccidentDegree === 1) {
          this.tempSIRNo = this.tempSIRNo + "J";
        } else if (this.strTypeOfReport === 3 && this.strAccidentDegree === 2) {
          this.tempSIRNo = this.tempSIRNo + "N";
        } else if (this.strTypeOfReport === 3 && this.strAccidentDegree === 3) {
          this.tempSIRNo = this.tempSIRNo + "";
        }
        if (this.strTypeOfReport === 4 && this.strAccidentDegree === 1) {
          this.tempSIRNo = this.tempSIRNo + "J";
        } else if (this.strTypeOfReport === 4 && this.strAccidentDegree === 2) {
          this.tempSIRNo = this.tempSIRNo + "N";
        } else if (this.strTypeOfReport === 4 && this.strAccidentDegree === 3) {
          this.tempSIRNo = this.tempSIRNo + "";
        }

        if (
          this.strTypeOfReport === undefined ||
          this.strTypeOfReport === null ||
          this.strTypeOfReport === "" ||
          this.strTypeOfReport === 0 ||
          this.strAccidentDegree === undefined ||
          this.strAccidentDegree === null ||
          this.strAccidentDegree === "" ||
          this.strAccidentDegree === 0 ||
          this.datehappened === undefined ||
          this.datehappened === null ||
          this.datehappened === "" ||
          this.datehappened === 0 ||
          this.datehappened === moment(this.datehappened).format("YYYYMMDD")
        ) {
          this.sirno = undefined;
        } else {
          axios
            .get(
              `${this.api}/getseqno/${this.datehappened}/${this.SIdatabank_userInfo.CompanyCode}`
            )
            .then((res) => {
              let Data = res.data[0];
              ...
              this.sirno =
                this.tempSIRNo +
                moment(this.datehappened).format("YYYYMMDD") +
                "-" +
                Data.SeqID;
              resolve(this.sirno);
              console.log("Seq", this.SeqID);
            });
        }
      });
    },
      ...
      if (tempEmployeeCode != "") {
        axios.get(`${this.EmployeeInfoURL}/${this.EmployeeID1}`).then((res) => {
          ...
          if (tempEmployeeCode != "" && this.infoData != undefined) {
            this.GetDepartments(this.infoData.ShortName);
            if (this.infoData.DepartmentName != undefined) {
              let modifiedDeptName = this.infoData.DepartmentName.replaceAll(" ", "%20");
          ...
          if (result.isConfirmed) {
            ...
            if (this.file.length === 0) {
              ...
              let image = {
                ...
                SirNo: this.sirno,
                image: `//hv12-hrd41/D/Deployment/SafetyInvestigatoryDatabank/NoPicture.jpg`,
    ...
    GetGroupOfCompanies() {
      axios
        .get(
          `${this.leadersURL}/${this.SIdatabank_userInfo.CompanyCode}/${this.SIdatabank_userInfo.DepartmentCode}`
        )
        .then((res) => {
