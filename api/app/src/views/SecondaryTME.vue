    ...
    <v-main>
        ...
        <v-container fluid>
            <v-card class="mt-3" v-if="isloading== false" :class="[isloading == false ? `imagehover` : '']">
                <v-row>
                    <v-col>
                        <v-card-subtitle class='ml-5  pb-7 mr-4 pt-5 ' style="font-size:20px">Secondary TME</v-card-subtitle>
                    </v-col>
                            ...
                            <v-form ref="form" v-model="valid" lazy-validation class="mt-4">
                                <v-row align="center" justify="center" dense>  
                                    <v-col v-if="editMode == true" cols="11">
                                        <v-text-field  
                                            v-model="toModify.SecondaryTMEID"                    
                                            label="ID"
...
export default{
    data() {
        return {
            ...
            headers:[
                { text: "ID", value: "SecondaryTMEID" },
                { text: "Classification Name", value: "SecondaryTME" },
    ...
    computed:{
        filteredData(){
            if(!this.search.toLowerCase()){
                ...
                return this.SecondaryTMEItems.filter(rec=>{
                return rec.SecondaryTME.includes(this.search)
                })
            }
        }, 
        GV_UserID: {
    ...
    methods: { 
        ...
        getSecondaryTME(){
            this.isloading = true
            this.axios.get(`${this.api}/getsecondarytmeNULL`).then(res=>{
                this.SecondaryTMEItems=res.data
        ...
        saveRecords() {                       
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/updatesecondarytme`,this.toModify).then(() => {
        ...
        addRecords(){
            this.toModify.SecondaryTMEID = this.SecondaryTMEItems.length + 1,
            ...
            this.swal.fire(this.saveOptions).then(result => {
            if(result.isConfirmed){
                this.axios.post(`${this.api}/addsecondarytme`,this.toModify).then(() => {
                this.swal.fire('Hooray!','Changes has been saved', 'success')
                this.dialog=false
                this.editMode = false
                this.getSecondaryTME() 
        ...
        deleteData(item) {
                this.swal.fire(this.saveOptions).then(result => {
                    if(result.isConfirmed){
                        this.axios.post(`${this.api}/deletesecondarytme`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getSecondaryTME() 
        ...
        ActivateData(item){
            this.swal.fire(this.saveOptions).then(result => {
                if(result.isConfirmed){
                        this.axios.post(`${this.api}/activesecondarytme`,item).then(() => {
                        this.swal.fire('Hooray!','Changes has been saved', 'success')
                        this.dialog=false
                        this.editMode = false
                        this.getSecondaryTME() 
